...
jQuery(document).ready(function(){
	jQuery('.btn.btn-warning').click(function(event){
...
function first(param, sync){
	...
	jQuery('#item-current > h4').html(list.head.data.name);
	jQuery('#item-current > span').html(list.head.data[param]);
	jQuery('.by-' + param + '.item-last > h4').html(list.head.next.data.name);
	jQuery('.by-' + param + '.item-last > span').html(list.head.next.data[param]);	
	list.currentNode = list.head;
	jQuery('#item-current > img').attr('src', list.currentNode.data.img);
...
function last(param, sync){
	...
	jQuery('#item-current > h4').html(list.tail.data.name);
	jQuery('#item-current > span').html(list.tail.data[param]);
...
function prev(param, sync){
	...
	if (list.currentNode.previous) {
		...
		jQuery('#item-current > h4').html(list.currentNode.previous.data.name);
		jQuery('#item-current > span').html(list.currentNode.previous.data[param]);			
...
function next(param, sync){
	...
	if (list.currentNode.next) {
		...
		jQuery('#item-current > h4').html(list.currentNode.next.data.name);
		jQuery('#item-current > span').html(list.currentNode.next.data[param]);			
...
function setCurrent(param, node){
	...
	if (node) {
		...
		jQuery('#item-current > h4').html(node.data.name);
		jQuery('#item-current > span').html(node.data[param]);			
